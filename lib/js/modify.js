    ...
    if (markers) {
        ...
        for (var i = 0; i < records; i++) {
            data[i] = {
                ...
                hidden: markerNodes[i].getAttribute("hidden"),
...
function display(data) {
    ...
    answer1.value = decodeHTMLEntities(data[pos].answer1);
    answer2.value = decodeHTMLEntities(data[pos].answer2);
    answer3.value = decodeHTMLEntities(data[pos].answer3);
    answer4.value = decodeHTMLEntities(data[pos].answer4);
    correct.value = data[pos].correct;
    hidden.value = data[pos].hidden;
}
...
function addQuiz(category) {
    ...
    document.getElementById('legend').innerHTML = "Add Trivia Question to " + category + " Category!";
...
function retrieveCategories() {
    form_data = serializeFormById('add-categories-form');
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            if (json.status === 'edit') {
                ...
                editQuiz('triviaxmledit.php', 0, json.total, json.category);
            }
            if (json.status === 'new') {
